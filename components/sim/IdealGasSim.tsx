    ...
    <View style={styles.container}>
      <View style={[styles.callout, { backgroundColor: colors.surface, borderColor: colors.border }]}>
        <Text style={[styles.hint, { color: colors.text }]}>
          PV = nRᵤT for an ideal gas. Adjust moles, temperature, and volume to see how pressure
          responds. Rᵤ = 8314 J/(kmol·K).
        </Text>
      </View>
      ...
      <View style={[styles.readout, { borderColor: colors.border, backgroundColor: colors.surface }]}>
        <Text style={[styles.pLabel, { color: colors.textMuted }]}>Pressure P</Text>
        <Text style={[styles.pMain, { color: colors.tint }]}>{P_kPa.toFixed(1)} kPa</Text>
        <Text style={[styles.pSub, { color: colors.textSecondary }]}>≈ {P_atm.toFixed(3)} atm</Text>
      </View>

      <Text style={[styles.barCap, { color: colors.textSecondary }]}>Relative pressure (caps at 5 MPa)</Text>
      <View style={styles.barTrack} onLayout={onBarLayout}>
        <Svg width={boxW} height={22}>
          ...
          <Rect x={2} y={2} width={barInnerW} height={18} rx={6} fill={colors.tint} />
        </Svg>
      </View>

      <Text style={[styles.barCap, { color: colors.textSecondary }]}>Volume container (wider = larger V)</Text>
      <Svg width={boxW} height={svgH}>
        <Rect x={0} y={0} width={boxW} height={svgH} fill={colors.chartFill} stroke={colors.border} rx={12} />
        <Rect
          x={containerX}
          y={containerY}
          width={containerW}
