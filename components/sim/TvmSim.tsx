    ...
    <View style={styles.container}>
      <View
        ...
        <Text style={[styles.hint, { color: colors.text }]}>
          Compound interest: F = P(1+i)ⁿ. Slide the rate or periods and watch the future value and
          factor table update.
        </Text>
      </View>
      ...
      <View style={[styles.readout, { borderColor: colors.border, backgroundColor: colors.surface }]}>
        <View style={styles.readRow}>
          <Text style={[styles.readLabel, { color: colors.textMuted }]}>Future Value F</Text>
          <Text style={[styles.readVal, { color: colors.tint }]}>${fv.toFixed(2)}</Text>
        </View>
        <View style={styles.readRow}>
          <Text style={[styles.readLabel, { color: colors.textMuted }]}>Total Interest</Text>
          <Text style={[styles.readVal, { color: colors.warning }]}>${totalInterest.toFixed(2)}</Text>
        </View>
      </View>

      <View style={[styles.factorBox, { borderColor: colors.border, backgroundColor: colors.surface }]}>
        <Text style={[styles.factorTitle, { color: colors.text }]}>Interest Factors</Text>
        <View style={styles.factorGrid}>
          <FactorRow label="(F/P, i, n)" value={fpFactor(i, n)} colors={colors} />
          <FactorRow label="(P/F, i, n)" value={pfFactor(i, n)} colors={colors} />
      ...
      <Text style={[styles.barCap, { color: colors.textSecondary }]}>Balance over {n} periods</Text>
      <View style={styles.barTrack} onLayout={onBarLayout}>
        <Svg width={barW} height={barH}>
          <Rect x={0} y={0} width={barW} height={barH} rx={8} fill={colors.chartFill} stroke={colors.border} />
          {n > 0 && (
            <Line
              x1={barPad}
...
function FactorRow({
  ...
  return (
    <View style={styles.fRow}>
      <Text style={[styles.fLabel, { color: colors.textSecondary }]}>{label}</Text>
      <Text style={[styles.fVal, { color: colors.tint }]}>{value.toFixed(4)}</Text>
    </View>
  );
}
